...
void t1_solver(FILE *fpOut, ProbInfo **prob_node){      
    ...
    for(i = 0; i < (*prob_node)->diamond_size - 1; i++){
        ...
        if(diamond_vect[i]->energy <= 0) break; // check if the sum of the positives is already smaller than the target
      
        sum_positives += diamond_vect[i]->energy;
        
    }

    (*prob_node)->max_pocket = sum_positives + pocket; // upper bound is computed
        
    // if max pocket is smaller or equal to zero, OR smaller than target, problem has no solution, there's no need to search

    if(((*prob_node)->max_pocket <= 0) || ((*prob_node)->max_pocket < target) ){
...
void t2_solver(FILE *fpOut, ProbInfo **prob_node) {
    ...
    for(i = 0; i < (*prob_node)->diamond_size - 1; i++){
        ...
        if(diamond_vect[i]->energy <= 0) break; // check if the sum of the positives is already smaller        
...
int Thereishope(ProbInfo **prob_node, stat_cell***diamond_vect, int next_pocket, int target, int steps2take, int next_line, int next_col){
 
    int i, distance, sum_positives = 0;

    for(i = 0; i < (*prob_node)->diamond_size - 1; i++){

        if((*diamond_vect)[i]->energy <= 0) break;

        if((*prob_node)->reduced_map[(*diamond_vect)[i]->rm_row][(*diamond_vect)[i]->rm_col]->inStack == 1) continue;

        distance = dist(next_line, next_col, (*diamond_vect)[i]->rm_row, (*diamond_vect)[i]->rm_col);

        if(0 < distance && distance <= steps2take){
    ...
    return (sum_positives + next_pocket) < target ? 0 : 1;
}
